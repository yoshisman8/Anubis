...
namespace Anubis.Modules
{
	public class Settings_Module : InteractiveBase<SocketCommandContext>
	{
		...
		public async Task Help()
		{
			...
			sb.AppendLine("**Join** <tile> <Initiative> - Join a battle and place yourself in the selected tile.");
			sb.AppendLine("**AddNPC** <Tile> <Initiative> <Token Image Url> <Name> - Adds an NPC to the encounter in the selected tile.");
		...
		public async Task Help([Remainder] string Topic)
		{
			...
			switch (Topic.ToLower())
			{
				...
				case "combat":
					...
					embed.AddField("Joining in as a player", "As a player, you can join an ongoing encounter by using the `Join <Tile> <Initiative>` command. You will have to manually specify your initiative value and will likely have to roll it beforehand.");
					embed.AddField("Adding NPCs as the director", "As the director, you can add NPCs to the encounter by using the `AddNPC <Tile> <Initiative> <Token image URL> <Name>`. You *must* supply a token image in order to add an NPC. If you add two NPCs with the same name, the bot will overwritte the existing entry with the new one.");
